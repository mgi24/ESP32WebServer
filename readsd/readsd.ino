...
#define MAX_PAGE 100
std::vector<String> imagefiles[MAX_PAGE];
void scanImagesPerPage()
{
  for (int page = 1; page <= imgfoldercount && page < MAX_PAGE; ++page)
  {
    imagefiles[page].clear();
    String pageDir = "/images/" + String(page);
    if (SD_MMC.exists(pageDir))
    {
      ...
      while (entry)
      {
        if (!entry.isDirectory())
        {
          Serial.println("Found image file in page " + String(page) + ": " + String(entry.name()));
          imagefiles[page].push_back(String(entry.name()));
        }
  ...
  if (SD_MMC.exists("/videos"))
  {
    ...
    while (entry)
    {
      ...
      if (!entry.isDirectory())
      {
        ...
        videosize.push_back(String(entry.size()));
      }
...
void sdScan()
{
  Serial.println("scan root");
  scanRoot();
  Serial.println("Scanimg");
  scanImgPage();
  Serial.print("ScanImagesPerPage");
  scanImagesPerPage();
...
void serverSetup()
{
  ...
  server.on("/files", HTTP_GET, [](AsyncWebServerRequest *request)
    ...
    for (size_t i = 0; i < rootfiles.size(); ++i) {
      if (i > 0) json += ",";
      json += "{\"name\":\"" + rootfiles[i] + "\",\"size\":" + rootsize[i] + "}";
    }
    json += "]";
  ...
  // Handle normal file manager
  server.on("/filemanager", HTTP_GET, [](AsyncWebServerRequest *request)
            { led_on();
    Serial.println("Received request for /filemanager");
    request->send(200, "text/html", normalmanager_html);
    led_off();
  });
  ...
  server.on("/videofiles", HTTP_GET, [](AsyncWebServerRequest *request)
    ...
    for (size_t i = 0; i < videolist.size(); ++i) {
      if (i > 0) json += ",";
      json += "{\"name\":\"" + videolist[i] + "\",\"size\":" + videosize[i] + "}";
    }
    json += "]";
    request->send(200, "application/json", json);
    led_off();
  ...
  server.on("/video", HTTP_GET, [](AsyncWebServerRequest *request)
            { led_on();
    Serial.println("Received request for /video");
    request->send(200, "text/html", video_html);
    led_off();
  });

  // IMAGE
  server.on("/imagepage", HTTP_GET, [](AsyncWebServerRequest *request)
            {
    led_on();
    Serial.println("Received request for /imagepage");
    String json = "{\"imgfoldercount\":" + String(imgfoldercount) + "}";
    request->send(200, "application/json", json);
    led_off();
  });

  // Handle image files per page
  server.on("/image", HTTP_GET, [](AsyncWebServerRequest *request)
            {
    Serial.println("Received request for /image");

    if (!request->hasParam("page")) {
      request->send(400, "application/json", "{\"error\":\"Missing page parameter\"}");
      return;
    }
    int page = request->getParam("page")->value().toInt();
    Serial.println("Requested page: " + String(page));
    if (page < 1 || page > imgfoldercount) {
      request->send(400, "application/json", "{\"error\":\"Invalid page number\"}");
      return;
    }
    String json = "[";
    for (size_t i = 0; i < imagefiles[page].size(); ++i) {
      if (i > 0) json += ",";
      json += "\"" + imagefiles[page][i] + "\"";
    }
    json += "]";
    request->send(200, "application/json", json); });
  ...
  server.on("/gallery", HTTP_GET, [](AsyncWebServerRequest *request)
            {
    led_on();
    Serial.println("Received request for /gallery");
    request->send(200, "text/html", gallery_html);
    led_off();
  });
  ...
  server.on(
      ...
      [](AsyncWebServerRequest *request, String filename, size_t index, uint8_t *data, size_t len, bool final)
      {
        ...
        if (index == 0)
        {
          Serial.println("Starting upload of: " + filename);
          String ext = filename.substring(filename.lastIndexOf('.') + 1);
          ext.toLowerCase();
          if (ext == "mp4")
          { 
            Serial.println("Detected video file, saving to /videos/");
          ...
          {
            Serial.println("File already exists, removing: " + path);
            SD_MMC.remove(path);
          }
          else{
            Serial.println("Add to videolist: " + filename);
            videolist.push_back(filename);
            videosize.push_back("0");
          }
